...
#define largebin_index_32(sz)                                                                                                                              \
    (((((unsigned long)(sz)) >> 6) <= 38) ? 56 + (((unsigned long)(sz)) >> 6) : ((((unsigned long)(sz)) >> 9) <= 20) ? 91 + (((unsigned long)(sz)) >> 9)   \
...
#define largebin_index_32_big(sz)                                                                                                                          \
    (((((unsigned long)(sz)) >> 6) <= 45) ? 49 + (((unsigned long)(sz)) >> 6) : ((((unsigned long)(sz)) >> 9) <= 20) ? 91 + (((unsigned long)(sz)) >> 9)   \
                                                                            : ((((unsigned long)(sz)) >> 12) <= 10)  ? 110 + (((unsigned long)(sz)) >> 12) \
                                                                            : ((((unsigned long)(sz)) >> 15) <= 4)   ? 119 + (((unsigned long)(sz)) >> 15) \
                                                                            : ((((unsigned long)(sz)) >> 18) <= 2)   ? 124 + (((unsigned long)(sz)) >> 18) \
                                                                                                                     : 126)
...
mremap_chunk(mchunkptr p, size_t new_size)
{
    ...
    INTERNAL_SIZE_T new;
    new = atomic_fetch_add_relaxed(&mp_.mmapped_mem, new_size - size - offset) + new_size - size - offset;
...
tcache_key_initialize(void)
{
    ...
    while (tcache_key <= 0x1000000 || tcache_key >= ((uintptr_t)ULONG_MAX) - 0x1000000 || stdc_count_ones(tcache_key) < minimum_bits || stdc_count_ones(tcache_key) > maximum_bits)
    {
...
{
    size_t idx = TCACHE_SMALL_BINS + __builtin_clz(MAX_TCACHE_SMALL_SIZE) - __builtin_clz(nb);
    return idx;
}
...
tcache_get_large(size_t tc_idx, size_t nb)
{
    ...
    if ((mangled && REVEAL_PTR(*entry) == NULL) || (!mangled && *entry == NULL))
        return NULL;
...
tcache_get_align(size_t nb, size_t alignment)
{
    if (nb < mp_.tcache_max_bytes)
    {
        ...
        while (te != NULL && ((csize = chunksize(mem2chunk(te))) < nb || (csize == nb && !PTR_IS_ALIGNED(te, alignment))))
        {
            tep = &(te->next);
            te = REVEAL_PTR(te->next);
            mangled = true;
       ...
       initialized in the loop above.  */
        ...
        if (te != NULL && csize == nb && PTR_IS_ALIGNED(te, alignment))
    ...
    void *__libc_realloc(void *oldmem, size_t bytes)
{
    ...
    if (__glibc_unlikely((uintptr_t)oldp > (uintptr_t)-oldsize || misaligned_chunk(oldp)))
...
#define REMOVE_FB(fb, victim, pp)                                          \
    ...
    } while ((pp = atomic_compare_and_exchange_val_acq(fb, pp, victim)) != victim);
    ...
    if (in_smallbin_range(nb))
    {
        ...
        if ((victim = last(bin)) != bin)
        {
            ...
            if (tcache != NULL && tc_idx < mp_.tcache_small_bins)
            {
                ...
                while (tcache->num_slots[tc_idx] != 0 && (tc_victim = last(bin)) != bin)
                {
                    if (tc_victim != NULL)
                    {
                        bck = tc_victim->bk;
...
_int_free_chunk(mstate av, mchunkptr p, INTERNAL_SIZE_T size, int have_lock)
{
    ...
    */
    ...
    )
    {
        ...
        if (__glibc_unlikely(
                chunksize_nomask(chunk_at_offset(p, size)) <= CHUNK_HDR_SZ || chunksize(chunk_at_offset(p, size)) >= av->system_mem))
        {
            bool fail = true;
        ...
        /* Check that size of fastbin chunk at the top is the same as
           ...
           allocated again.  */
        if (have_lock && old != NULL && __glibc_unlikely(fastbin_index(chunksize(old)) != idx))
...
_int_free_merge_chunk(mstate av, mchunkptr p, INTERNAL_SIZE_T size)
{
    ...
    if (__glibc_unlikely(contiguous(av) && (char *)nextchunk >= ((char *)av->top + chunksize(av->top))))
    ...
    if (__glibc_unlikely(chunksize_nomask(nextchunk) <= CHUNK_HDR_SZ || nextsize >= av->system_mem))
             ...
             INTERNAL_SIZE_T nb)
{
    ...
    /* oldmem size */
    if (__glibc_unlikely(chunksize_nomask(oldp) <= CHUNK_HDR_SZ || oldsize >= av->system_mem || oldsize != chunksize(oldp)))
    ...
    if (__glibc_unlikely(chunksize_nomask(next) <= CHUNK_HDR_SZ || nextsize >= av->system_mem))
...
_int_memalign(mstate av, size_t alignment, size_t bytes)
{
    ...
    if (!chunk_is_mmapped(p))
    {
        ...
        {
            remainder_size = size - nb;
            if (remainder_size >= MINSIZE || nextchunk == av->top || !inuse_bit_at_offset(nextchunk, nextsize))
            {
...
do_set_hugetlb(size_t value)
{
    if (value == 1)
    {
        ...
        if (mp_.thp_mode == malloc_thp_mode_madvise || mp_.thp_mode == malloc_thp_mode_always)
...
int __posix_memalign(void **memptr, size_t alignment, size_t size)
{
    ...
    if (alignment % sizeof(void *) != 0 || !powerof2(alignment / sizeof(void *)) || alignment == 0)
        return EINVAL;
...
#define nsizes (sizeof(sizes) / sizeof(sizes[0]))
       ...
       comments.  */
        ...
        for (size_t i = 1; i < NBINS; ++i)
        {
            ...
            sizes[NFASTBINS - 1 + i].to = sizes[NFASTBINS - 1 + i].total = sizes[NFASTBINS - 1 + i].count = 0;
            ...
            if (r != NULL)
                while (r != bin)
                {
                    ...
                    sizes[NFASTBINS - 1 + i].from = MIN(sizes[NFASTBINS - 1 + i].from, r_size);
